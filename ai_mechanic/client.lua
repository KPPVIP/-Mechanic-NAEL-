...
RegisterNetEvent('ai_mechanic:openMenu')
AddEventHandler("ai_mechanic:openMenu", function()
...
function GoToTarget(x, y, z, vehicle, driver, vehhash, target)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "Mechanic Dispatched", "Un mécanicien a été envoyé sur place. Merci d'utiliser ~y~" .. companyName)
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(target, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "Véhicule réparé" , mechPedPick.lines[math.random(#mechPedPick.lines)])
...
function GetTargetVehicle(player, dir)
    ...
    if DoesEntityExist(dmgVeh) then
        ...
        ShowNotification("Impossible de trouver un véhicule.")
    end
end

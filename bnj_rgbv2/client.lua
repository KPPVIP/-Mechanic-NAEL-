...
Citizen.CreateThread(function()	
	...
	while true do
		...
		if plate == nil then
			...
			ESX.TriggerServerCallback("esx_bnj:SyncRGB", function(isPurchasable)
...
RegisterNetEvent("esx_bnj:rgb")
AddEventHandler("esx_bnj:rgb", function()
	...
	if IsPedInVehicle(ped, car, false) then
		...
		ESX.ShowAdvancedNotification('~r~Attention', '', "~r~Vous devez vous asseoir dans la voiture pour changer la couleur des lumières", pic, 1)
		PlaySoundFrontend(-1, "Highlight_Error", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 0)
	end
end)
...
RegisterNUICallback('save', function(data, cb)
	...
	SetNuiFocus(false, false)
	PlaySoundFrontend(-1, "Continue_Accepted", "DLC_HEIST_PLANNING_BOARD_SOUNDS", 0)
	SendNUIMessage({type = 'close'})
	MenuGUI = false
	...
	TriggerServerEvent('esx_bnj:Sync')
	ESX.TriggerServerCallback("esx_bnj:pharesrgb", function(isPurchasable)
		if isPurchasable then
			ESX.ShowAdvancedNotification('~g~Tablette RGB', '', "~y~Les ampoules ont été installées", pic, 1)
	
		else
			ESX.ShowAdvancedNotification('~g~PTabletteRGB', '', "~y~Une erreur s'est produite", pic, 1)
		end
...
RegisterNUICallback('change_color_blue', function(data, cb)
	...
	PlaySoundFrontend(-1, "SKIP", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0)
end)

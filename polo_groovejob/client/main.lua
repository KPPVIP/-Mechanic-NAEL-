...
function OpenMobilegrooveActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_groove_actions', {
		...
		elements = {
			...
			{label = '👨‍🏭 Call a groove',       value = 'meca1'},	
			{label = '📣 Ad Open ',     value = 'mecano_ouvert'},
			{label = '📣 Ad Close',     value = 'mecano_ferme'},
			{label = '📣 Ad Pause',     value = 'mecano_pause'},
			{label = '📣 Ad Promo',     value = 'mecano_promo'},
			...
			{label = '🏷 Plate Menu', value = 'platemenu'}
	}}, function(data, menu)
		if isBusy then return end
			...
			if data.current.value == 'meca1' then
				TriggerEvent('ai_mechanic:openMenu')	
				menu.close()
        end

			if data.current.value == 'nitro' then
				TriggerEvent('SB_Carnitro:openMenuPolo')	
				menu.close()
        end
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'billing', {
				...
				if amount == nil or amount < 0 then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						menu.close()
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_groove', _U('groove'), amount)
					end
				end
			end, function(data, menu)
				menu.close()
		...
		elseif data.current.value == 'mecano_pause' then
			TriggerServerEvent('AnnounceMecanoPause')
		elseif data.current.value == 'mecano_promo' then
			TriggerServerEvent('AnnounceMecanoPromo')
		...
		elseif data.current.value == 'hijack_vehicle' then
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
				exports['progressBars']:startUI(10000, "Hijack..")
				Citizen.Wait(10000)
	...
	elseif data.current.value == 'fix_vehicle' then
		...
		elseif data.current.value == 'cctv' then	
			...
			local elements = {					
				...
				{label = '🎥 : Camera Interior 1', value = 'cam6'},
				{label = '🎥 : Camera Interior 2', value = 'cam7'},	
				{label = '🎥 : Camera Interior 3', value = 'cam8'},	
				{label = '🎥 : Camera Interior 4', value = 'cam9'},
				{label = '🎥 : Camera Interior 5', value = 'cam10'},
				--{label = '🎥 : Caméra Intérieur 1', value = 'cam9'},
				{label = '🎥 : Camera Outside 1', value = 'cam4'},
				{label = '🎥 : Camera Outside 2', value = 'cam5'},
		...
		elseif data.current.value == 'platemenu' then	
			local elements  = {}

			local elements = {					
				{label = 'Remove the Plate', value = 'plate'},
				{label = 'Put the plate', value = 'plate1'},							
			}
			
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'platemenu', {
                css      = 'groove',
				title    = '🏷 Plate Menu',
				align    = 'top-left',
				elements = elements
				...
				if action == 'plate' then
					TriggerEvent('plateRemover:openRemove')  	
				elseif action == 'plate1' then
					TriggerEvent('plateput:openPlate') 					
				elseif action ==  'exit' then
					menu.close()					
				end
			end, function(data2, menu2)
...
AddEventHandler('esx_groovejob:hasEnteredEntityZone', function(entity)
	...
	if ESX.PlayerData.job and ESX.PlayerData.job.name == 'groove' and not IsPedInAnyVehicle(playerPed, false) then
...
-- Create Blips
Citizen.CreateThread(function()
	local blip = AddBlipForCoord(Config.Zones.grooveActions.Pos.x, Config.Zones.grooveActions.Pos.y, Config.Zones.grooveActions.Pos.z)
	...
	AddTextComponentSubstringPlayerName(_U('groove'))
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and ESX.PlayerData.job and ESX.PlayerData.job.name == 'groove' then
				...
				elseif CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						if
							...
							GetEntityModel(vehicle) == GetHashKey('slamvan')
						then
		...
		if IsControlJustReleased(0, 167) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'groove' then
			OpenMobilegrooveActionsMenu()
		end

		if IsControlJustReleased(0, 178) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'groove' then
			if NPCOnJob then

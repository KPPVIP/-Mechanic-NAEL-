...
local nettoyage = {coords = vector3(-111.31, -1798.35, 26.20)}
local nettoyagePublique = {coords = vector3(0.0, 0.0, 0.0)}
     ...
     while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'groove' then
			if dstCheck <= 2.5 then
				sleepThread = 5
				ESX.Game.Utils.DrawText3D(nettoyage.coords, "~b~[E] ~w~Global vehicle repair \n~b~ Business activity", 1.0)
...
function NettoyageFunction()
	local ped = PlayerPedId()
     ...
     if IsPedInAnyVehicle(ped, false) then
          if ESX.PlayerData.job and ESX.PlayerData.job.name == 'groove' then
               ESX.ShowAdvancedNotification("Mechanic", "~b~Repair and cleaning", "Vehicle repair and cleaning in progress...", "CHAR_LS_CUSTOMS", 8)
-- Cam
               local camera = CreateCam("DEFAULT_SCRIPTED_CAMERA", 1)
               SetCamCoord(camera, -103.50, -1813.84, 30.0)
...
-- Fin cam
               ...
               ESX.ShowAdvancedNotification("Mechanic", "~b~Repair and cleaning", "Completed", "CHAR_LS_CUSTOMS", 8)
               -- porte
          ...
          ESX.ShowAdvancedNotification("Mechanic", "~b~Repair and cleaning", "You have to be in a vehicle to do this.", "CHAR_LS_CUSTOMS", 8)
     end
end
